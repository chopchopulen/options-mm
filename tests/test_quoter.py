...
from src.greeks.analytical import gamma as G, vega as V
...
def _q(base_spread=0.05):
    return Quoter(base_spread=base_spread, holding_horizon=TAU,
                  vol_of_vol=XI, contract_size=100)


def test_bid_below_ask():
    bid, ask = _q().quote(fair_value=5.0, gamma=0.02, vega=10.0, S=450.0, sigma=0.20)
    assert bid < ask
...
def test_wider_with_higher_gamma():
    lo = _q().half_spread(gamma=0.01, vega=0.0, S=450.0, sigma=0.20)
    hi = _q().half_spread(gamma=0.10, vega=0.0, S=450.0, sigma=0.20)
    assert hi > lo


def test_wider_with_higher_vega():
    lo = _q().half_spread(gamma=0.0, vega=10.0, S=450.0, sigma=0.20)
    hi = _q().half_spread(gamma=0.0, vega=100.0, S=450.0, sigma=0.20)
    assert hi > lo
...
def test_gamma_charge_equals_carry_cost_over_tau():
    ...
    charge = _q(base_spread=0.0).half_spread(gamma=g, vega=0.0, S=S, sigma=sigma)
...
def test_vega_charge_equals_heston_vol_move_over_tau():
    ...
    v = 61.79
    charge = _q(base_spread=0.0).half_spread(gamma=0.0, vega=v, S=450.0, sigma=0.20)
...
def test_half_spread_is_low_single_digit_percent_of_premium():
    ...
    for K, days, otype in [(427.5, 30, "put"), (450.0, 30, "call"), (450.0, 30, "put"),
        ...
        hs = _q().half_spread(G(S, K, T, r, sigma), V(S, K, T, r, sigma), S, sigma)
        pct = hs / premium
        assert pct < 0.10, f"{K} {days}d {otype}: half-spread is {pct:.1%} of premium"
